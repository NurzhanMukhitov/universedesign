...
/*
 * Собранный куб покачивается, а не крутится по кругу.
 *
 * Два требования спорят между собой: куб должен быть живым и при этом
 * всегда выглядеть одинаково. Обычное вращение выполняет первое и рушит
 * второе — за десяток секунд оно уводит грани на двадцать градусов,
 * и ровного ракурса как не бывало.
 *
 * Поэтому вместо оборота — качание вокруг фронтального положения. Размах
 * подобран замером: при семи градусах правая грань подходила ближе левой
 * и становилась заметно ярче — центр яркости уезжал на 86 пикселей, и куб
 * читался перекошенным, хотя геометрически оставался симметричным.
 * Два градуса дают жизнь, но ракурс держат.
 */
const SWAY_AMPLITUDE = 0.035;
const SWAY_PERIOD = 45000;
...
export class CubeEngine {
    ...
    render(time) {
        ...
        const sway = this.dragging
            ? 0
            : Math.sin(((time - this.startedAt) / SWAY_PERIOD) * Math.PI * 2) *
              SWAY_AMPLITUDE;
        ...
        const sinY = Math.sin(this.rotationY + sway);
        const cosY = Math.cos(this.rotationY + sway);
