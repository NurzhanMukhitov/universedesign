...
export class CubeEngine {
    ...
    render(time) {
        ...
        /*
         * Куб крутится всегда — и собранный тоже: он живой объект, а не
         * картинка. Молчит только пока идёт переход, иначе автоповорот
         * перебивал бы угол, который переход как раз и выставляет, — он
         * применяется следующей строкой после него.
         *
         * Одинаковость сборки от этого не страдает: переход приводит куб
         * в исходный разворот, и каждая сборка начинается с одного и того же
         * ракурса. Дальше он снова медленно едет.
         */
