...
/*
 * Куб медленно вращается.
 *
 * Качание с размахом в пару градусов, которое стояло здесь до этого, читалось
 * как неподвижность — владелец так и сказал: не крутится. Возвращаем оборот,
 * но втрое медленнее оригинального: полный круг за полторы минуты.
 *
 * Одинаковость сборки это не ломает. Переход приводит куб точно на фронт,
 * и каждая сборка начинается с эталонного ракурса — дальше он уезжает,
 * но к следующей сборке снова вернётся.
 */
const AUTO_ROTATE_SPEED = 0.00007;
...
export class CubeEngine {
    constructor(canvas) {
        ...
        // Начало отсчёта качания. Без него синус берётся от абсолютного
        // времени, которое на момент загрузки уже ненулевое, — и куб
        // появляется чуть повёрнутым вместо строго фронтального.
        this.startedAt = null;
    ...
    stepTransition(time) {
        ...
        /*
         * Ход ровный, без разгона в середине.
         *
         * На боевом сайте интерполяция линейная: tParam растёт равномерно
         * все полторы секунды. Здесь стояла кубическая ease-in-out — она
         * проходит середину вдвое быстрее линейной, и разлёт читался
         * как рывок: тронулся, дёрнул, замер.
         *
         * Оставляем лёгкое торможение у цели — точки садятся мягко, — но
         * старт и середина идут ровно, как в оригинале.
         */
    ...
    render(time) {
        ...
        /*
         * Качание живёт отдельно от угла, который крутит человек пальцем.
         *
         * Оно не накапливается в rotationY, а добавляется поверх при отрисовке.
         * Иначе пришлось бы выбирать: либо куб дышит и уезжает, либо стоит
         * ровно и мёртв. А так угол сборки остаётся тем же самым, и качание
         * всегда идёт вокруг него.
         */
        if (!this.dragging && !this.transition) {
        ...
        for (let i = 0; i < LINES_X; i++) {
            for (let j = 0; j < LINES_Y; j++) {
                for (let k = 0; k < pointsPerLine; k++) {
                    ...
                    } else if (gathered && k === pointsPerLine - 1) {
                        /*
                         * Белым — ближняя к зрителю грань.
                         *
                         * В оригинале белым красился ряд j === 13, то есть
                         * одна горизонтальная линия: там куб стоял под углом,
                         * и этого хватало. При фронтальном ракурсе так белой
                         * оказывалась полоска посередине, а не грань. Эталон,
                         * который показал владелец, светится по всем четырём
                         * краям — это передний слой по глубине.
                         */
                        ctx.font = `bold ${font}px OneDay`;
