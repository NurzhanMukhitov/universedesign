...
export function Cube({ scattered = false, reducedMotion = false }) {
    ...
    useEffect(() => {
        ...
        // Полторы секунды — как на боевом сайте, но там разлёт запускается
        // рывком колеса и на его фоне читается быстрее, чем есть. Здесь
        // курок спускает обычная прокрутка, и на этой длительности движение
        // выглядело торопливым. Две с половиной секунды — тот же жест,
        // досказанный до конца.
        engine.animateTo(scattered ? 1 : 0, 2500);
