...
/*
 * Круговой режим: текст собирается и рассыпается без конца. Нужен, только
 * чтобы разглядеть эффект, не прокручивая страницу туда-сюда.
 *
 * На сайте выключен: постоянное движение рядом с текстом, который надо
 * прочитать, читать мешает. Поставить true — и цикл вернётся.
 */
const LOOP = false;
    ...
    return useMemo(() => {
        ...
        return text.split(' ').map((word, wordIndex) => ({
            ...
            letters: [...word].map((char) => ({
                ...
                /*
                 * Задержка перед стартом буквы.
                 *
                 * Держим её маленькой намеренно: чем теснее задержки, тем
                 * больше букв летит одновременно — а именно этого просил
                 * владелец. При широком разбросе половина уже сидит на месте,
                 * пока вторая не тронулась, и в кадре пусто.
                 *
                 * Верхняя граница связана с длительностью фазы в CSS: их
                 * сумма обязана равняться единице. Иначе буквы с самой
                 * поздней задержкой не успевают долететь — при 0.72 и фазе
                 * 0.34 они замирали на 82% пути, и в тексте зияли дыры.
                 */
                delay: random() * 0.25,
            })),
        }));
