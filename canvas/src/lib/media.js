...
/*
 * Логотип студии — тот же файл, что на боевом сайте. Набирать его шрифтом
 * нельзя: начертание авторское, и текстовая имитация выдаёт себя
 * расстояниями между буквами.
 *
 * Импорт статический, а не через new URL(import.meta.url): пререндер идёт
 * в Node, где import.meta.url — это путь в файловой системе, и в готовый
 * HTML попадал бы адрес вида file:///assets/… Статический импорт Vite
 * подменяет на настоящий путь ещё на сборке.
 *
 * Через imagetools не гоняем: это белая графика на прозрачном фоне,
 * в avif и webp она выиграет считанные килобайты, а риск получить кайму
 * по краю выше выгоды.
 */
export { default as wordmarkUrl } from '../../../logo/UNIVERSE.png';
